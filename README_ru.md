...
Статические файлы могут быть отдны через стандартные обаботчики `routing.Static` или `routing.StaticFile`. Первый используется для отдачи файла из указанной папки с файлами в соответствии с текущим запросом, а второй для конкретного заданного файла. Например,
